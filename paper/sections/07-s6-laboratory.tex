...
\item paper-grade positive examples, such as the double-coset construction
      above;
\item compact obstruction families under development;
...
\[
        A=HK\sqcup H\tau K,
\]
with $H\cong D_4$ and $K\cong S_3$.  The displayed generators suggest a
right-hand matching pattern in the cover graph of the poset, while the
left-hand symmetry organizes the target labels.
...
\texttt{s6\_classification\_status.json}.  After forcing the identity
translate, the audit computation
