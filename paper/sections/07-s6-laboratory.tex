...
\text{unresolved survivors in the audit} & 0.
\end{array}
\]
...
The known non-series-parallel tilers in $S_6$ include multiplier sets with
subgroup, coset, and double-coset structure.
...
On the negative side, most obstruction-side candidates fall into reusable
...
\texttt{03-certificates/s6\_classification\_status.json}.  The canonical
...
appendix.  After forcing the identity translate, the residual packing graph
has maximum clique one below the size required for a tiling.  In the table
below, ``req.'' is the number of tiles required for a full tiling,
...
\end{array}
\]
Equivalently, each maximum near-packing leaves a residual set of exactly
one tile-size, but the residual cannot be completed by another translate.
